...
-- Function to create workouts table
CREATE OR REPLACE FUNCTION create_workouts_table()
RETURNS void AS $$
BEGIN
...
-- Function to create journal_entries table
...
-- Function to set up all RLS policies
CREATE OR REPLACE FUNCTION setup_rls_policies()
RETURNS void AS $$
BEGIN
  -- This function is a wrapper to call all the table creation functions
  PERFORM create_workouts_table();
  PERFORM create_exercises_table();
  PERFORM create_journal_entries_table();
END;
$$ LANGUAGE plpgsql;

-- Create a function to be called via RPC to set up all database functions
CREATE OR REPLACE FUNCTION create_database_functions()
RETURNS void AS $$
BEGIN
  -- This function is already created by running this script
  RAISE NOTICE 'Database functions created successfully';
END;
$$ LANGUAGE plpgsql;

-- Call the setup function to create all tables and policies
